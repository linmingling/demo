...
class ShexController extends ControllerBase {
    ...
    public function indexAction(){
        ...
        $list_sql = "SELECT * FROM shex ORDER BY add_time DESC LIMIT ".$page->firstRow.','.$page->listRows;
...
//         echo '<pre>';print_r($list);exit;
    ...
    public function reserveAction(){
        //数据列表
        $sql = "SELECT COUNT(*) FROM shex_subscribe";
        $count = $this->group->fetchOne($sql);
    
        $page = new \Library\Com\Page($count[0], 10);
        $list_sql = "SELECT * FROM shex_subscribe ORDER BY add_time DESC LIMIT ".$page->firstRow.','.$page->listRows;
        $list = $this->group->fetchAll($list_sql);
        //         echo '<pre>';print_r($list);exit;
        $this->view->setVar('page',$page->show());
        $this->view->setVar("list", $list);
	...
	public function bmtable_xlsAction(){
	    ...
	    $list_sql = "SELECT * FROM shex ".$where." ORDER BY add_time DESC";
        $list = $this->group->fetchAll($list_sql);
	    ...
	    $filename = "shanghai-".date('Ymd').'.xls';
	...
	public function bmtable_allxlsAction(){
	    ...
	    $list_sql = "SELECT * FROM shex ORDER BY add_time DESC";
	    $list = $this->group->fetchAll($list_sql);
    ...
    public function searchAction(){
        ...
        $sql = "SELECT COUNT(*) FROM shex WHERE ".$where;
        $count = $this->group->fetchOne($sql);
        
        
        $page = new \Library\Com\Page($count[0], 10);
        $list_sql = "SELECT * FROM shex WHERE ".$where." ORDER BY add_time DESC LIMIT ".$page->firstRow.','.$page->listRows;
        $list = $this->group->fetchAll($list_sql);
        $this->view->setVar('page',$page->show());
        $this->view->setVar("list", $list);
    ...
    public function budian_statisticsAction(){
        ...
        $bm_sql = "SELECT COUNT(*) FROM shex WHERE btn_id <> 0";
        $bm_count = $this->group->fetchOne($bm_sql);
    
        $sql = "SELECT COUNT(*) FROM shex_budian";
        $count = $this->group->fetchOne($sql);
        ...
        //每天报名转化率
        $list_sql = "SELECT * FROM shex_budian WHERE add_time > ".$zuijin_time." ORDER BY add_time DESC";
        $list = $this->group->fetchAll($list_sql);
        foreach ($list as $k => $key){
            if($key['add_time']){
...
//        echo '<pre>';print_r($list_arr);exit;
        ...
        $bm_sql = "SELECT * FROM shex WHERE btn_id <> 0 AND add_time > ".$zuijin_time;
...
//        echo '<pre>';print_r($arr);exit;
        ...
        //产品图布点数据统计
        $sql = "SELECT * FROM shex_goods_click";
        $click_list = $this->group->fetchAll($sql);
        foreach ($click_list as $ck => $ckey){
            $sql = "SELECT COUNT(*) FROM shex_goods_budian WHERE btn_id='".$ckey['btn_id']."'";
            $count = $this->group->fetchOne($sql);
            $click_list[$ck]['num'] = $count[0];
        }
        $this->view->setVar('click_list', $click_list);
//         echo '<pre>';print_r($click_list);exit;
