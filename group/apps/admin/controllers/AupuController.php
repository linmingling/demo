...
class AupuController extends ControllerBase {
    ...
    public function indexAction(){
        ...
        //数据列表
        $sql = "SELECT COUNT(*) FROM aupu_registration".$where;
        $count = $this->group->fetchOne($sql);
        
        $mb_sql = "SELECT COUNT(*) FROM aupu_registration WHERE btn_id <> 0";
        $mb_count = $this->group->fetchOne($mb_sql);
        
        $page = new \Library\Com\Page($count[0], 10);
        $list_sql = "SELECT * FROM aupu_registration ".$where." ORDER BY add_time DESC LIMIT ".$page->firstRow.','.$page->listRows;
    ...
    public function budianAction(){
        ...
        $sql = "SELECT COUNT(*) FROM aupu_budian";
        $count = $this->group->fetchOne($sql);
        $page = new \Library\Com\Page($count[0], 10);
        $list_sql = "SELECT * FROM aupu_budian ORDER BY add_time DESC LIMIT ".$page->firstRow.','.$page->listRows;
        $list = $this->group->fetchAll($list_sql);
        foreach ($list as $k => $key){
    ...
    public function baoming_statisticsAction(){
        ...
        $sql = "SELECT COUNT(*) FROM aupu_registration";
        $count = $this->group->fetchOne($sql);
        //数据统计
        $group_sql = "SELECT city, COUNT(city_code) AS num FROM aupu_registration GROUP BY city_code";
        $group_list = $this->group->fetchAll($group_sql);
        
        $success_sql = "SELECT city, COUNT(city_code) AS num FROM aupu_registration WHERE state=1 GROUP BY city_code";
        ...
        //每天各站报名数量情况
        $bm_sql = "SELECT * FROM aupu_registration ORDER BY add_time DESC";
...
//         echo '<pre>';print_r($list);exit;
    ...
    public function budian_statisticsAction(){
        ...
        $bm_sql = "SELECT COUNT(*) FROM aupu_registration WHERE btn_id <> 0";
        ...
        //各屏布点数据统计
        $group_sql = "SELECT btn_id, COUNT(btn_id) AS num FROM aupu_budian GROUP BY btn_id";
        $group_list = $this->group->fetchAll($group_sql);
        
        $city_zhl_sql = "SELECT btn_id, COUNT(btn_id) AS num FROM aupu_registration WHERE btn_id <> 0 GROUP BY btn_id";
...
//         echo '<pre>';print_r($bdzhl_arr);exit;
        ...
        //各城市站布点数据统计
        $city_sql = "SELECT city, COUNT(city_code) AS num FROM aupu_budian GROUP BY city_code";
       ...
       //每天报名转化率
       $list_sql = "SELECT * FROM aupu_budian WHERE add_time > ".$zuijin_time." ORDER BY add_time DESC";
       $list = $this->group->fetchAll($list_sql);
       foreach ($list as $k => $key){
           if($key['add_time']){
               $time_k = date('m月d日', $key['add_time']);
...
//        echo '<pre>';print_r($list_arr);exit;
       ...
       $bm_sql = "SELECT * FROM aupu_registration WHERE btn_id <> 0 AND add_time > ".$zuijin_time;
       $bm_list = $this->group->fetchAll($bm_sql);
       foreach ($bm_list as $k => $key){
           if($key['add_time']){
               $time_k = date('m月d日', $key['add_time']);
...
//        echo '<pre>';print_r($arr);exit;
	...
	public function bmtable_xlsAction(){
	    ...
	    $list_sql = "SELECT * FROM aupu_registration ".$where." ORDER BY add_time DESC";
        $list = $this->group->fetchAll($list_sql);
    ...
    public function searchAction(){
        ...
        $sql = "SELECT COUNT(*) FROM aupu_registration WHERE ".$where;
        $count = $this->group->fetchOne($sql);
        
        $mb_sql = "SELECT COUNT(*) FROM aupu_registration WHERE btn_id <> 0 AND ".$where;
        $mb_count = $this->group->fetchOne($mb_sql);
        
        $page = new \Library\Com\Page($count[0], 10);
        $list_sql = "SELECT * FROM aupu_registration WHERE ".$where." ORDER BY add_time DESC LIMIT ".$page->firstRow.','.$page->listRows;
        $list = $this->group->fetchAll($list_sql);
        $this->view->setVar('page',$page->show());
        $this->view->setVar("list", $list);
        $this->view->setVar("mb_count", $mb_count[0]);
