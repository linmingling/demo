...
class HebController extends ControllerBase {
    ...
    public function indexAction(){
        ...
        setcookie('search');
        //数据列表
        $sql = "SELECT COUNT(*) FROM heb";
        $count = $this->group->fetchOne($sql);
        
        $page = new \Library\Com\Page($count[0], 10);
        $list_sql = "SELECT * FROM heb ORDER BY add_time DESC LIMIT ".$page->firstRow.','.$page->listRows;
...
//         echo '<pre>';print_r($list);exit;
    ...
    public function budian_statisticsAction(){
        ...
        $bm_sql = "SELECT COUNT(*) FROM heb WHERE btn_id <> 0";
        $bm_count = $this->group->fetchOne($bm_sql);
    
        $sql = "SELECT COUNT(*) FROM heb_budian";
        $count = $this->group->fetchOne($sql);
        ...
        $start_time = '2015-09-23';
        $time = date('Y-m-d', time());
        $for_num = (strtotime($time) - strtotime($start_time))/86400;//活动进行天数
        $zuijin_time = strtotime($time) - 86400 * 5;//最近5天的数据
         
        //每天报名转化率
        $list_sql = "SELECT * FROM heb_budian WHERE add_time > ".$zuijin_time." ORDER BY add_time DESC";
        $list = $this->group->fetchAll($list_sql);
        foreach ($list as $k => $key){
            if($key['add_time']){
...
//        echo '<pre>';print_r($list_arr);exit;
        ...
        $bm_sql = "SELECT * FROM heb WHERE btn_id <> 0 AND add_time > ".$zuijin_time;
