...
class MeiligeController extends ControllerBase {
    ...
    public function indexAction(){
        ...
        //数据列表
        $sql = "SELECT COUNT(*) FROM meilige_registration";
        $count = $this->group->fetchOne($sql);
        
        $mb_sql = "SELECT COUNT(*) FROM meilige_registration WHERE btn_id <> ''";
        $mb_count = $this->group->fetchOne($mb_sql);
        
        $page = new \Library\Com\Page($count[0], 10);
        $list_sql = "SELECT * FROM meilige_registration ORDER BY add_time DESC LIMIT ".$page->firstRow.','.$page->listRows;
    ...
    public function budian_statisticsAction(){
        ...
        $bm_sql = "SELECT COUNT(*) FROM meilige_registration WHERE btn_id <> ''";
        $bm_count = $this->group->fetchOne($bm_sql);
        
        $sql = "SELECT COUNT(*) FROM meilige_budian";
        $count = $this->group->fetchOne($sql);
        ...
        //各屏布点数据统计
        $group_sql = "SELECT btn_id, COUNT(btn_id) AS num FROM meilige_budian GROUP BY btn_id";
        $group_list = $this->group->fetchAll($group_sql);
        
        $city_zhl_sql = "SELECT btn_id, COUNT(btn_id) AS num FROM meilige_registration WHERE btn_id <> '' GROUP BY btn_id";
...
//         echo '<pre>';print_r($bdzhl_arr);exit;
       ...
       //每天报名转化率
       $list_sql = "SELECT * FROM meilige_budian WHERE add_time > ".$zuijin_time." ORDER BY add_time DESC";
       $list = $this->group->fetchAll($list_sql);
...
//        echo '<pre>';print_r($list_arr);exit;
       ...
       $bm_sql = "SELECT * FROM meilige_registration WHERE btn_id <> '' AND add_time > ".$zuijin_time;
...
//        echo '<pre>';print_r($arr);exit;
	...
	public function bmtable_xlsAction(){
	    ...
	    $list_sql = "SELECT * FROM meilige_registration ".$where." ORDER BY add_time DESC";
        $list = $this->group->fetchAll($list_sql);
	...
	public function bmtable_allxlsAction(){
	    ...
	    $list_sql = "SELECT * FROM meilige_registration ORDER BY add_time DESC";
	    $list = $this->group->fetchAll($list_sql);
    ...
    public function searchAction(){
        ...
        $sql = "SELECT COUNT(*) FROM meilige_registration WHERE ".$where;
        $count = $this->group->fetchOne($sql);
        
        $mb_sql = "SELECT COUNT(*) FROM meilige_registration WHERE btn_id <> '' AND ".$where;
        $mb_count = $this->group->fetchOne($mb_sql);
        
        $page = new \Library\Com\Page($count[0], 10);
        $list_sql = "SELECT * FROM meilige_registration WHERE ".$where." ORDER BY add_time DESC LIMIT ".$page->firstRow.','.$page->listRows;
        $list = $this->group->fetchAll($list_sql);
        $this->view->setVar('page',$page->show());
        $this->view->setVar("list", $list);
        $this->view->setVar("mb_count", $mb_count[0]);
