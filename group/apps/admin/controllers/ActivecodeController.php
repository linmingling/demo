...
class ActivecodeController extends ControllerBase {
    ...
    public function indexAction(){
        setcookie('select');
        setcookie('search');
        $sql = "SELECT COUNT(*) FROM qx_activecard_msg";
        $count = $this->group->fetchOne($sql);
        $page = new \Library\Com\Page($count[0], 10);
        $list_sql = "SELECT * FROM qx_activecard_msg order by id desc LIMIT ".$page->firstRow.','.$page->listRows;
    ...
    public function searchAction(){
        ...
        if($this->request->isPost() == true){
            ...
            {
            	$p = explode(',', $search);
            	$where = "card_number in(". "select card_number from qx_membership_card where ".$select.">".$p[0]." and " . $select . "<" . $p[1] .")";
            }
            else 
            {
            	$where = $select."='".$search."'";
        ...
        $sql = "SELECT COUNT(*) FROM qx_activecard_msg WHERE ".$where;
        $count = $this->group->fetchOne($sql);
    
    
        $page = new \Library\Com\Page($count[0], 10);
        $list_sql = "SELECT * FROM qx_activecard_msg WHERE ".$where." order by id desc LIMIT ".$page->firstRow.','.$page->listRows;
        $list = $this->group->fetchAll($list_sql);
        ...
        $this->view->pick("activecode/index");
    }
    ...
    public function bmtable_xlsAction(){
        ...
        if(!empty($search) && !empty($search)){
        	...
        	{
        		$p = explode(',', $search);
        		$where = ' WHERE '."card_number in(". "select card_number from qx_membership_card where ".$select.">".$p[0]." and " . $select . "<" . $p[1] .")";
        	}
        	else
        	{
        ...
        $str = "城市\t卡号\t姓名\t电话\t红包金额\t微信名\t微信识别号\t激活时间\t\n";
        $str = iconv('utf-8','gb2312',$str);
         
        $list_sql = "SELECT * FROM qx_activecard_msg ".$where;
        $list = $this->group->fetchAll($list_sql);
        ...
        foreach ($list as $k => $key){
            ...
            $money_status = iconv('utf-8','gb2312//IGNORE',$key['money_status']);
            $wechaname = iconv('utf-8','gb2312//IGNORE',$key['wechaname']);
            $openid = iconv('utf-8','gb2312//IGNORE',$key['openid']);
            $mem_active = iconv('utf-8','gb2312//IGNORE',$key['mem_active']);
            $str .= $city."\t".$card_number."\t".$name."\t".$phone."\t".$money_status."\t".$wechaname."\t".$openid."\t".$mem_active."\t\n";
        }
        $filename = 'activecode_'.date('Ymd').'.xls';
