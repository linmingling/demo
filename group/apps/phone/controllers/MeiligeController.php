...
//         echo '<pre>';print_r($sql);exit;
    ...
    public function submitAction(){
        ...
        $ip = $this->session->get('meilige_ip');//ip地址
        ...
        $sql = "SELECT * FROM meilige_registration WHERE phone='".$phone."'";
        $data = $this->group->fetchOne($sql);
        if(!empty($data)){
            if($data['state'] == 1){
                $sql = "UPDATE meilige_registration SET num=num+1 WHERE phone='".$phone."'";
                $result = $this->group->execute($sql);
		        $ajax_result['errcode'] = 1002;
		        $ajax_result['errmsg'] = "您已成功报名，请勿重复报名！";
		    } else {
		        $sql = "UPDATE meilige_registration SET name='".$name."' WHERE phone='".$phone."'";
		        $result = $this->group->execute($sql);
		        if($result){
            ...
            $sql = "INSERT INTO meilige_registration (name, phone, ip_city, url_source, ip, btn_id, verify, verify_time, update_verify_time, add_time, num) VALUES('".$name."','".$phone."','".$ip_city."','".$url_source."','".$ip."','".$btn_id."','0','0','0','".time()."','1')";
            $this->group->execute($sql);
            $resultId = $this->group->lastInsertId();
            if($resultId){
                $this->post_crm(50, $name, $phone, '');
                $ajax_result['errcode'] = 0;
                $ajax_result['errmsg'] = "信息提交成功！";
            } else {
                $ajax_result['errcode'] = 1003;
    ...
    public function getVerifyAction(){
        ...
        if(!empty($data)){
            if($data['state'] == 1){
                ...
                $sql = "UPDATE meilige_registration SET verify='".$verify."',update_verify_time='".time()."' WHERE phone='".$phone."'";
                $result = $this->group->execute($sql);
                if($result){
                    $ajax_result['errcode'] = 0;
    ...
    public function submit_codeAction(){
        ...
        if(!empty($data)){
            if($data['state'] == 1){
                ...
                $sql = "UPDATE meilige_registration SET address='".$address."',area='".$area."',state=1,verify_time='".time()."' WHERE phone='".$phone."'";
	            $result = $this->group->execute($sql);
                if($result){
                    $this->post_crm(50, '', $phone, $address);
                    $ajax_result['errcode'] = 0;
    ...
    public function clickAction(){
        if($this->request->isAjax()){
            ...
            $sql = "INSERT INTO meilige_budian (btn_id, url_source, add_time) VALUES('".$id."','".$url_source."','".time()."')";
            $this->group->execute($sql);
        }
    }
    ...
    public function sendAction(){
        ...
        if(empty($verify)){
            $content = '【腾讯优居】恭喜您成功领取11月14日-15日美力格工厂店床垫团购会2000元抵扣券以及免费门票一张，您可以凭借此短信作为领取凭证，使用抵扣券。地址：深圳龙岗区宝雅5号-雅兰工厂美力格展厅工作人员将于24小时内与您电话联系确认地址并邮寄门票，我们会以 010开头固话呼出，注意接听哟。回T退订';
