...
//         echo '<pre>';print_r($address['content']['address']);exit;
    ...
    public function getVerifyAction(){
        ...
        $ip = $this->session->get('sh_ip');//ip地址
        ...
        if(!empty($data)){
            ...
            $sql = "INSERT INTO shex (name, phone, ip, ip_address, url_source, btn_id, verify, verify_time, update_verify_time, add_time) VALUES('".$name."','".$phone."','".$ip."','".$address."','".$url_source."','".$btn_id."','".$verify."','0','".time()."','".time()."')";
    ...
    public function clickAction(){
        if($this->request->isAjax()){
            ...
            $sql = "INSERT INTO shex_budian (btn_id, add_time) VALUES('".$id."','".time()."')";
            $this->group->execute($sql);
        }
    }
    
    //产品页面布点
    public function goods_clickAction(){
        if($this->request->isAjax()){
            $id = trim($_POST['btn_id']);
            $phone = trim($_POST['tel']);
            
            $sql = "SELECT * FROM shex_goods_budian WHERE phone='".$phone."' AND btn_id='".$id."'";
            $data = $this->group->fetchOne($sql);
            if(empty($data)){
                $sql = "INSERT INTO shex_goods_budian (btn_id, phone, add_time) VALUES('".$id."','".$phone."','".time()."')";
                $this->group->execute($sql);
            }
            $sql = "UPDATE shex_goods_click SET click_num=click_num+1 WHERE btn_id='".$id."'";
            $this->group->execute($sql);
            echo 0;exit;
        }
    }
