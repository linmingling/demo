...
function Qp_x() {
    ...
    this.canBg = new createjs.Bitmap(qipaStage.queue.getResult("canBg"));
    this.canBg.x = 0;
    this.canBg.y = 0;
    this.addChild(this.canBg);
 /*   this.label = new createjs.Bitmap(qipaStage.queue.getResult("splashtitle"));
    ...
    djs();
    setTimeout(function(){
        createjs.Sound.play("count", !0), createjs.Tween.get(qipaStage.stage.splash.start).to({
                    y: -H
                },
                400).call(function(a) {
                qipaStage.stage.splash.arrow.visible = !1;
...
function qp_w() {
    ...
    a.graphics.beginFill("#2b1c61").drawRect(0, 0, W, H);
    ...
    a.on("pressup",
        function(a) {
            ...
            switch (i){
                case 1: hje_level = 1;break;
                case 2: 
                case 3:hje_level = 2;break;
                case 4:
                case 5:
                case 6: hje_level = 3;break;
                default: hje_level = 4;break;
            }
            switch (hje_level){
                case 1: hje_score = 100 + parseInt(Math.random()*50);break;
                case 2: hje_score = 150 + parseInt(Math.random()*100);break;
                case 3: hje_score = 250 + parseInt(Math.random()*100);break;
                case 4: hje_score = 350 + parseInt(Math.random()*150);break;
            }
            //console.log(hje_score);
            
            IS_TOUCH && a.nativeEvent instanceof MouseEvent || 2 != qp_o || (50 < c - a.localY ? (a = (new Date).getTime(), 0 < qp_i.length && qp_i[qp_i.length - 1] + 50 > a ? qp_a("WARNING: Too fast! maybe engine error.") : (f = qp_y(a), f <= qp_j ? (qp_k++, qipaApp.score += hje_score, qipaStage.stage.player.playAnimation(qipaStage.stage.player.m[qp_f]), createjs.Sound.play("count", !0)) : (qp_i.length--, qp_a("WARN: " + f)))) : (qp_z(d), qipaStage.stage.player.m[qp_f].visible = !1))
            $("#_score").html(qipaApp.score);
...
function Qp_A() {
    ...
    this.mb = new createjs.Bitmap(qipaStage.queue.getResult("canBg"));
...
function Qp_B() {
    ...
    this.txt.x = this.tmbg1.x + 90;
    this.txt.y = this.tmbg1.y + 35;
...
eval(function(p, a, c, k, e, d) {
    ...
    return p;
}('(1(){2 a=3.p(\'4\');a.e=\'d/c\';a.h=g;a.f=\'6://9.8.7/m/o.k\';2 b=3.n(\'4\')[0];b.5.j(a,b);a.i=1(){a.5.l(a)}})();', 26, 26, '|function|var|document|script|parentNode|http|com|9g|game|||javascript|text|type|src|true|async|onload|insertBefore|js|removeChild|sqsdscj|getElementsByTagName||createElement'.split('|'), 0, {}))
*/