(function() {
	...
	game.showUI = function() {
		...
		if (_this.playBtn==null) {
			...
			playBtn.setUpState({rect : [0, 0, 139, 43]});
			...
			if (dx <= mainRole.getCurrentWidth() + hW && dx >= 0 && dy <= 2*hH && dy >= -hH - 100) {
                ...
                switch(ball.name){
                    ...
                    case "pao":
                        ...
                        break;
                    case "feiji":
                        _this.gamesIcon(0,true,"feiji");
                        break;
                    case "da":
                        _this.gamesIcon(3,true,"da");
                        break;
                    case "ttfc":
                        _this.gamesIcon(4,true,"ttfc");
                        break;
                }
                balls[i].alpha=0;
				return true;
			}
            if(Q.getDOM("head")){Q.getDOM("head").style.backgroundPosition="0px -202px";}
    ...
    game.countDown=function(a){
        ...
        game.timeId = setTimeout(function(){
        	...
        	document.getElementById("djs").innerHTML = game.seconds;
