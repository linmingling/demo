...
if($_POST)
{
	...
	if($act == 'addinfo')
	{
        ...
        $check_sql = "select * from $user_table where phone='{$phone}'";
		$check_res = mysqli_query($db,$check_sql);
		$check_row = $check_res->fetch_assoc();
        if(empty($check_row))
        {
            $sql = "INSERT INTO $user_table(add_time, name, phone, province,city,address) VALUES('" . date('Y-m-d H:i:s') . "','{$name}','{$phone}','{$province}','{$city}','{$address}')";
	        mysqli_query($db, $sql);
        }
	...
	if($act == 'start')
	{
		...
		if($check_quantity_row['quantity'] == 0 && in_array($arr_m[$pn],$p_list))
		{
			...
			$ajax_result['errmsg'] = 'ok';
			$ajax_result['prize'] = 0;
			mysqli_query($db,"update $user_table set is_prize=1 where phone='{$phone}'");
			die(json_encode($ajax_result));
		}
		else
		{
			...
			switch ($arr_m[$pn]) {
				case 1:
					mysqli_query($db,"update $user_table set is_prize=1,prize_name='吸尘器',prize=1,prize_code=$code where phone='{$phone}'");
					break;
				case 4:
					mysqli_query($db,"update $user_table set is_prize=1,prize_name='四件套',prize=4,prize_code=$code where phone='{$phone}'");
					break;
				case 7:
					mysqli_query($db,"update $user_table set is_prize=1,prize_name='豆芽机',prize=7,prize_code=$code where phone='{$phone}'");
					break;
				case 10:
					mysqli_query($db,"update $user_table set is_prize=1,prize_name='门票',prize=10,prize_code=$code where phone='{$phone}'");
					break;
				default:
					mysqli_query($db,"update $user_table set is_prize=1 where phone='{$phone}'");
					break;
...
// 		print_r($pn);die;
		...
		$sql = "update $user_table set q_$code=1,times=times-1 where phone='{$phone}'";
		mysqli_query($db, $sql);
	...
	if($act == 'check')
	{
		$check_sql = "select * from $user_table where phone='{$phone}' and phone>0";
		$check_res = mysqli_query($db,$check_sql);
		$check_row = $check_res->fetch_assoc();
	...
	if($act == 'addtimes')
	{
		$sql = "update $user_table set times=times+1 where phone='{$phone}'";
		mysqli_query($db,"begin");
		mysqli_query($db,$sql);
		mysqli_query($db,"commit");
