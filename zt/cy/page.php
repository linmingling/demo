...
class Page {
    ...
    public function go_prev() {
        $html="";
        if ($this->page==1) {
            $html.="<a class='pgn select2'>{$this->config['prev']}</a>";
        }else{
            $html.="<a class='pgn' href='{$this->uri}&page=" . ($this->page - 1) . "'>{$this->config['prev']}</a>";
        }
        return $html;
    }
    public function go_next() {
        $html="";
        if ($this->page==$this->total_pages || ($this->total_pages==0 && $this->page==1)) {
            $html.="<a class='pgn select2'>{$this->config['next']}</a>";
        }else{
            $html.="<a class='pgn' href='{$this->uri}&page=" . ($this->page + 1) . "'>{$this->config['next']}</a>";
        }
        return $html;
    }
    ...
    public function page_list() {
        ...
        for ($i = $i_num; $i >= 1; $i--) {
            ...
            if ($page<1) {
                ...
                $link_page.="<a class='pg' href='{$this->uri}&page={$page}'>{$page}</a>";
            }
        }
        $link_page.="<a class='pg select'>{$this->page}</a>";
        for ($i = 1; $i < $i_num; $i++) {
            $page=$this->page+$i;
