...
if(!$live_user){
	$url = 'http://'.$_SERVER['HTTP_HOST']."/live3/login.php";
	header("Location: $url");
}
...
if($edit_id && $id){
	...
	if(empty($arr)){
		header("location: /live3/admin.php");exit;
	} else {
		$arr_info['title'] = '';
		$arr_info['info'] = '';
	}
...
<div id="floatPanel"> 
    <div class="ctrolPanel" style="right:20px;"> 
        ...
        <a class="contact" href="/live3/notice.php" target="_blank"><span>公告</span></a> 
...
$(function(){
		 	...
		 	success:function(result){
		 		...
		 		if(info_list){
					...
					for(var j=0;j<info_list.length;j++){
						htmlblock = '<p class="grey"><span class="list_time">发布时间：'+info_list[j]['md_time']+info_list[j]['hi_time']+
						...
						<?php if($auid == 0){?>
							htmlblock += '<span class="list_edit edit" id="'+info_list[j]['id']+'"><a>编辑</a></span>';
						<?php }?>
						htmlblock += '</p>';
						$(".news_list").append(htmlblock);
						/* $(".news_list").append(
				   ...
				   $.ajax({
						async:false,
					 	...
					 	success:function(result){
							...
							location.href = "/live3/add.php?id="+id;
					 	}
					});
				});
			});
	  ...
	  function edit(){
		  $('.news_list .edit').each(function(index){
			  $(this).click(function(){
				  ...
				  location.href = "/live3/add.php?id="+id+"&edit_id="+edit_id;
				});
			});
	  }
	...
	$("#box1 .bmBtn_fh").click(function(){
		<?php if($edit_id){
			echo 'location.href = "/live3/add.php?id='.$id.'"';
		} else {
			echo 'location.href = "/live3/admin.php"';
		}?>
	});
});
</script>
