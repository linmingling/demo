...
$(function(){
	...
	var setLayout = function () {
		...
		var sl = function () {
			...
			if($win.height()<860){
				...
				$(".foot_zt").css({position:"position",bottom:"0","z-index":"1"});
				$(".person_box").css({bottom:"-80px"});

			}else{
				_h = $win.height();
				$(".foot_zt").css({position:"fixed",bottom:"0","z-index":"1"});
				$(".person_box").css({bottom:"45px"});
			}
			$wrapers.height(_h);
			$container.height(_h);
			$container.width(_w);
	...
	chose = function(dom,i){
		...
		// $(dom+" .person_box .dialog .dialog_box").eq(0).fadeIn(500);
   ...
   //初始化
	...
	//转盘
/*
	$("#zhuanpan .zp_btn").click(function(){
//		star(4);
		$.ajax({
			async:false,
		 	url: 'server.php',
	...
	star = function(i){
		$("#zhuanpan .zp_btn").addClass("on");

		setTimeout(function(){
			$("#zhuanpan .zp_btn").removeClass("on").addClass("on"+i);
		},500);
