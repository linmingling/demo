...
//    })
  ...
  $('#done').click(function(){
    name = returnVal('name');
    tel = returnVal('phone');
    wish = returnVal('textmsg');
	fromId = returnVal('fromId');
	console.log(fromId);
        if(checkForm(name,tel,wish)){
             $.ajax({
               async:false,
                 url: 'server.php',
                 data:{act:'addinfo',name:name,phone:tel,content:wish,fromId:fromId},
                 type: "post",
                 dataType:'json',
  ...
  // 预约验证表单
    function checkForm(name,tel,wish){
