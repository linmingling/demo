...
if(isGet() == true) {
    ...
    $rsQ = '(
    SELECT A.id,A.name,A.phone,A.email,A.add_time,A.user_type,B.name as org_name FROM '. $this->table .' A LEFT JOIN tc_organization B
        ON A.org_id = B.id ' . $rsSql . $limit . '
    )';
