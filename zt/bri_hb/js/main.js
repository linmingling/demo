...
function Qp_x() {
	...
	this.hand = new createjs.Bitmap(qipaStage.queue.getResult("mb0"));
	this.hand.y = H - this.hand.getBounds().height + 50;
	this.hand.x = 0;
...
function qp_w() {
	...
	*/
	...
	function(a) {
		//加分
		IS_TOUCH && a.nativeEvent instanceof MouseEvent || 2 != qp_o || (50 < c - a.localY ? (a = (new Date).getTime(), 0 < qp_i.length && qp_i[qp_i.length - 1] + 50 > a ? qp_a("WARNING: Too fast! maybe engine error.") : (f = qp_y(a), f <= qp_j ? (qp_k++, qipaApp.score += 1, money = (parseFloat(money)+(Math.random()*0.8)).toFixed(2), qipaStage.stage.player.playAnimation(qipaStage.stage.player.m[qp_f]), createjs.Sound.play("count", !0)) : (qp_i.length--, qp_a("WARN: " + f)))) : (qp_z(d), qipaStage.stage.player.m[qp_f].visible = !1))
	...
	function(a) {
		...
		0 <= qp_m && (qp_m += a.delta, a = shijian - parseInt(qp_m / 1E3), a != qp_n && (qp_n = a, qipaStage.stage.num.txt.text = qp_n + '"'), 0 >= qp_n && (qp_m = -1, qp_t()));
		//qipaStage.stage.num.sum.text = "\uffe5" + qipaApp.score
		qipaStage.stage.num.sum.text = "\uffe5" + money
	})
}
function Qp_A() {
	this.initialize();
...
function Qp_C() {
	...
	var c = new createjs.Bitmap(qipaStage.queue.getResult("porket"));
	c.x = b.x;
	c.y = a.y + 380;
	...
	var d = new createjs.Bitmap(qipaStage.queue.getResult("use"));
	d.x = b.x;
	d.y = a.y + 220;
	...
	this.addChild(c);
	this.addChild(d);
	this.scoreText = new createjs.Text("", "bold 34px Arial", "#67332a");
	this.scoreText.textAlign = "center";
	this.scoreText.x = W / 2;
	this.scoreText.y = a.y + 50;
	this.addChild(this.scoreText);
	this.shareText = new createjs.Text("", "36px Arial", "#eee");
...
Qp_C.prototype.refresh = function() {
	this.scoreText.text = "本次成绩：" + qipaApp.score+"个   本次金额："+ money + "元";
	this.shareText.text = 0 < qipaApp.score ? qipaShare.desc.replace("\u6bd4", "\n\u6bd4").replace("\u6211\u662f", "\n\u6211\u662f") : ""
};
function Qp_B() {
	this.initialize();
	//分数
	this.tmbg = new createjs.Bitmap(qipaStage.queue.getResult("fbg"));
	//this.tmbg.x = (W - this.tmbg.getBounds().width) / 2;
	this.tmbg.x = 30;
	this.tmbg.y = 0;
	this.addChild(this.tmbg);
	this.sum = new createjs.Text("\uffe5" + money, "bold 46px Arial", "yellow");
	this.sum.textAlign = "center";
	this.sum.textBaseline = "middle";
	//this.sum.x = W / 2;
	this.sum.x = this.tmbg.getBounds().width / 2 +50;
	this.sum.y = this.tmbg.y + this.tmbg.getBounds().height / 2;
	this.addChild(this.sum);
	//时间
	this.tmbg1 = new createjs.Bitmap(qipaStage.queue.getResult("tbg"));
	...
	this.txt.x = this.tmbg1.x + this.tmbg.getBounds().width / 2;
